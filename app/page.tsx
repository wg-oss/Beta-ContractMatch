...
import type { User, Contractor, Realtor, Connection, MeetingRequest, PortfolioItem, Post, BookedSlot } from "../types"
...
const failingImageUrls: Record<string, string> = {
  ...
  "https://images.unsplash.com/photo-1600210492486-724fe5c67fb3?auto=format&fit=crop&q=80&w=500":
    "https://images.pexels.com/photos/279607/pexels-photo-279607.jpeg?auto=compress&cs=tinysrgb&w=500&h=500&fit=crop",
}
...
const updatedContractors: Contractor[] = contractors.map((contractor) => {
  if (contractor.id === "c4") {
    ...
    const updatedPosts =
      contractor.posts && contractor.posts.length > 0
...
export default function App() {
  ...
  const [currentUser, setCurrentUser] = useState<AppUser | null>(null) // Changed from realtors[0] to null
  ...
  const handleLogin = (userData: {
    ...
    const user =
      userData.role === "realtor"
        ...
        : contractors.find((c) => c.id === userData.id)
    ...
    if (scheduleContractor?.availability) {
      const updatedContractor: Contractor = {
        ...
        availability: {
          ...
          bookedSlots: [...scheduleContractor.availability.bookedSlots, { ...slot, notes: "" }],
        },
      }
      setContractorsData((prev) => prev.map((c) => (c.id === scheduleContractor.id ? updatedContractor : c)))
  ...
  const handleAddProject = (project: PortfolioItem) => {
    if (currentUser?.role === "contractor") {
      ...
      setContractorsData((prev) => prev.map((c) => (c.id === currentUser.id ? updatedContractor : c)))
      const updatedUser: AppUser = {
        ...currentUser,
        portfolio: updatedContractor.portfolio,
      }
  ...
  if (!isAuthenticated || !currentUser) {
    return (
      <LoginPage
        ...
        setTempUsers={
          setTempUsers as React.Dispatch<
          ...
          messages: [
            {
              ...
              senderId: "c1",
              content:
                "Hey Michael, I saw your post about investment properties. I'd love to collaborate on a renovation!",
  ...
  const renderContractorCard = (contractor: Contractor) => {
    ...
    return <ContractorCard contractor={contractor} currentUser={user} onSchedule={() => handleSchedule(contractor)} />
  }
          ...
          <div className="px-4 py-4 flex justify-between items-center">
            {activeSection === "messages" ? (
              <div className="flex items-center gap-2">
                <button onClick={() => setActiveSection("feed")} className="p-1 hover:bg-gray-100 rounded-full">
                  <ArrowLeft size={20} />
                </button>
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleBackFromProfile} className="p-1 hover:bg-gray-100 rounded-full">
                  <ArrowLeft size={20} />
                </button>
                <h1 className="text-xl font-semibold">{viewingUser?.name}</h1>
              </div>
            ) : (
              <h1 className="text-2xl font-bold text-blue-500 font-poppins tracking-tight">ContractMatch</h1>
            )}
              ...
              {(activeSection === "feed" ||
                ...
                <button
                  ...
                  className={`p-2 rounded-full ${
                    activeSection === "messages" ? "text-blue-500" : "text-gray-600"
                  } hover:text-gray-800`}
                >
            ...
            <div className="space-y-4">
              {userConversations.map((chat) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <img
                      src={chat.recipient.photo || "/placeholder.svg?height=40&width=40"}
